...
- Implementar pipeline de **visão computacional** em **C++** capaz de detectar e reconhecer rostos em tempo real, com latência inferior a 500ms;
- garantir que a catraca e o buzzer funcionem **independentemente de conectividade de rede**;
- Transmitir eventos de acesso ao backend via **protocolo MQTT** com garantia de entrega (QoS1);
...
6. Evento é publicado no broker MQTT (tópico _facegate/events/access_);
